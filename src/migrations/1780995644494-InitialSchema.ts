...
export class InitialSchema1780995644494 implements MigrationInterface {
    name = 'InitialSchema1780995644494'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`user_id\` varchar(36) NOT NULL, \`order_number\` varchar(255) NULL, \`status\` enum ('Placed', 'To Ship', 'Cancelled', 'To Receive', 'Completed', 'Return/Refund') NOT NULL DEFAULT 'To Ship', \`subtotal\` decimal(10,2) NOT NULL, \`shipping_fee\` decimal(10,2) NOT NULL, \`tax\` decimal(10,2) NOT NULL, \`promo_id\` varchar(36) NULL, \`total\` decimal(10,2) AS (subtotal + shipping_fee + tax) STORED NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_f9180f384353c621e8d0c414c1\` (\`order_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_827b957847e995225deeab790c\` ON \`review\``);
