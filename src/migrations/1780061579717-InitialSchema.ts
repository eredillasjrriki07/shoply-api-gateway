...
export class InitialSchema1780061579717 implements MigrationInterface {
    name = 'InitialSchema1780061579717'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order_payment\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`order_id\` int UNSIGNED NOT NULL, \`method\` enum ('Card') NOT NULL, \`status\` enum ('pending', 'authorized', 'paid', 'failed', 'refund_pending', 'refunded') NOT NULL DEFAULT 'pending', \`provider_ref\` varchar(36) NULL, \`amount\` decimal(10,2) NOT NULL, \`paid_at\` datetime NULL, UNIQUE INDEX \`REL_fb74ab8e4ee3d2c6e73c261d8e\` (\`order_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`order_timeline_event\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`order_id\` int UNSIGNED NOT NULL, \`event_type\` enum ('Placed', 'To ship', 'Shipped', 'Delivered', 'Return/Refund') NOT NULL DEFAULT 'Placed', \`note\` text NULL, \`occurred_at\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`user_id\` varchar(36) NOT NULL, \`order_number\` varchar(255) NULL, \`status\` enum ('To Ship', 'To Receive', 'Completed', 'Cancelled', 'Return/Refund') NOT NULL DEFAULT 'To Ship', \`subtotal\` decimal(10,2) NOT NULL, \`shipping_fee\` decimal(10,2) NOT NULL, \`tax\` decimal(10,2) NOT NULL, \`total\` decimal(10,2) AS (subtotal + shipping_fee + tax) STORED NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_f9180f384353c621e8d0c414c1\` (\`order_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
